...
- [ ] **B05 Record format.** Go types for the envelope; the JSON Schema; ADRs for open decisions
...
| 2026-09-20 | rename | The project was renamed from Sluiceway to Lawang (Javanese for door or gate: the thing every change passes through, and where it is decided who may see it). It had to happen before the repository went public and before v0.1 froze the names that reach operators and sinks: the Go module path, the binary, the `LAWANG_*` environment prefix, the Postgres schema, the three roles and the `lawang.tenant` setting. Nothing was deployed, so there is no migration path: an existing local database is bootstrapped again from scratch. Same commit: `SECURITY.md`, and the real tunnel hostname taken out of `CLAUDE.md`. |
