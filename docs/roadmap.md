...
| 12 | What orders the supersede chain, what the ledger holds, and what a masked value's placeholder is | **settled:** decimal counters by their number, equal-length versions by their bytes, anything else refused by name, then arrival order for two records that carry the same version; one row per record PREPARED, keyed per entity; and a random placeholder with the map kept locally, see [ADR 12](adr/0012-ledger-supersede-masking.md) | ADR 4 left "monotonic version" undefined for an opaque string, and a chain that can point backwards replaces a newer record at the sink together with the scope access is decided on |
